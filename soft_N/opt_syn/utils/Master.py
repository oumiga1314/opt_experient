...
import math
import random
from utils.select_time_and_k import select_wait
from opt_syn.utils.select_candiate_key import candicate_key
...
class Master1(tf_pb2_grpc.masterServicer):
    def __init__(self):
        ...
        self.agg_av_time=0 #用来记录平均聚合一个worker梯度的平均时间
   ...
   #接收Client来的通知开始训练
    ...
    def master_notice_ps(self):
        ...
        if self.k==1:
            ...
            # if(self.wait_syn_time<=(syn_end_time-self.record_syn_start))|(len(self.finish_ip)==self.worker_num):
            #     self.master_is_free = False
            #     #self.count = 0
            #     self.tmp_ip = self.finish_ip
            #     self.finish_ip = []
    ...
    def ps_notice_master_pull_data_finished(self, request, context):
        if self.k==1:
            self.agg_av_time=request.agg_time
            print(self.agg_av_time)
        #print(request.notice)
    ...
    def calc_wait_time(self):
        ...
        time_data.append(0.8)
        time_data.append(0.75)
        time_data.append(0.7)
        time_data.append(0.9)
        time_data.append(1)
        time_data.append(0.75)
        time_data.append(4.3)
        time_data.append(5)
        time_data.append(7)
        time_data.append(5.5)
        time_data.append(6)
        time_data.append(7.7)
        time_data.append(1.5)
        time_data.append(1.25)
        time_data.append(1.4)
        time_data.append(1.5)

        self.wait_syn_time,self.k=select_wait(time_data,self.k,self.agg_av_time,self.threshold)
