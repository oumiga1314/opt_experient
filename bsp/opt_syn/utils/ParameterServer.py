...
class ParameterServer(tf_pb2_grpc.psServicer):
    ...
    def aggregate_gradient(self):
        ...
        # 正式更新梯度
         ...
         if len(self.workers)==len(self.temped_ip):
             aggrate_time = (round(end_aggrate-start_aggrate,3))/len(self.workers)+0.1
             print(aggrate_time/len(self.workers))
         else:
             aggrate_time=0
         self.master.ps_notice_master_pull_data_finished(tf_pb2.note_agg_time(agg_time=aggrate_time))
